package com.partior.client.ui.util;
...
public class UIUtils {
	...
	public static FlexLayout createPhoneLayout(String numberValue) {
	...
	public static Component createTitle(VaadinIcon icon, String title) {
	...
	public static List<String> getBic(String centralBank, CantonDataService cantonDataService) throws Exception {
	...
	public static List<String> getShortName(String centralBank, CantonDataService cantonDataService) throws Exception {
		List<String> sponsors = new ArrayList<>();
		if(cantonDataService.getUserBankName().equals( cantonDataService.getCdbcOperator() )) {
			List<AccountOwnerResponseDto> accountOwnerResponseDtoList = cantonDataService.listAccountOwners(centralBank);
			  sponsors= accountOwnerResponseDtoList
					.stream().map(accDto -> accDto.getShortName()
					).collect(Collectors.toList());
		} else {
			sponsors.add( cantonDataService.getUserBankName());
		}
		return  sponsors;
	}

	public static List<String> getToBankShortName(String centralBank, CantonDataService cantonDataService) throws Exception {
		List<String> sponsors = new ArrayList<>();
	...
	public static List<String> getParticipantAccount(String bic, CantonDataService cantonDataService) throws Exception {
		List<AccountOwnerResponseDto> accountOwnerResponseDtoList = cantonDataService.listParticipantBankAccounts(bic);
		List<String> sponsors =  accountOwnerResponseDtoList
				.stream().map( accDto -> accDto.getAccountId()
				).collect(Collectors.toList());
		return  sponsors;
	}


	public static List<String> getRtgsIds(String bic, CantonDataService cantonDataService) throws Exception {
		List<AccountOwnerResponseDto> accountOwnerResponseDtoList = cantonDataService.listParticipantBankAccounts(bic);
		List<String> sponsors =  accountOwnerResponseDtoList
				.stream().map( accDto -> accDto.getRtgsAccountId()
				).collect(Collectors.toList());
		return  sponsors;
	}

	public static List<String> getCbdcAccountId(String shortName, CantonDataService cantonDataService) throws Exception {
		List<AccountOwnerResponseDto> accountOwnerResponseDtoList = cantonDataService.listParticipantBankAccounts(shortName);
		List<String> sponsors =  accountOwnerResponseDtoList
				.stream().map( accDto -> accDto.getAccountId()
				).collect(Collectors.toList());
		return  sponsors;
	...
	public static final Map<String, String> ENTITY_NAME = Map.of(
			"DBS", "Development Bank of Singapore",
			"UOB", "United Overseas Bank",
			"OCBC", "OCBC",
			"SGBANK1", "Singapore Bank 1",
			"SGBANK2", "Singapore Bank 2",
			"SGBANK3", "Singapore Bank 3",
			"INDOBANK1", "Indonesia Bank 1",
			"INDOBANK2", "Indonesia Bank 2",
			"INDOBANK3", "Indonesia Bank 3"
			);

	public static final Map<String, String> CBDC_NAME = Map.of(
	 		"MAS", "Monetary Authority of Singapore",
			"BOI", "Bank of Indonesia",
            "BI", "Bank of Indonesia"
	);

	public static final Map<String, String> CBDC_CURRENCY = Map.of(
			"MAS", "SGD",
			"BOI", "IDR",
            "BI", "IDR",
            "BSP", "PHP"
	...
	public static final Map<String, String> CBDC_OPERATOR = Map.of(
			"mas", "mas",
			"MAS", "mas",
			"boi", "boi",
			"BOI", "boi"
    ...
    public static Component stopLightIcon(String value){
        String statusColor = value.equalsIgnoreCase("ONLINE")
                ||  value.equalsIgnoreCase("CONNECT")
                ||  value.equalsIgnoreCase("TRUE") ?"green":"red";
        Icon statusIcon = new Icon(VaadinIcon.CIRCLE);
        statusIcon.setSize("20px");
        statusIcon.setColor(statusColor);
